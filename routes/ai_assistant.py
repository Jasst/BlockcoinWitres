...
class CognitiveController:
    ...
    def _build_messages(self, message: str, web_search: bool, search_context: str,
        ...
        if reasoning:
            # ИСПРАВЛЕНИЕ v2: локальные модели часто генерируют ТОЛЬКО рассуждение,
            # игнорируя инструкцию "затем дай финальный ответ". Убираем требование
            # точного формата с "---" и просим модель дать и рассуждение, и ответ.
            # Фронтенд сам распознает рассуждение по ключевым словам.
            system_parts.append(
                "Включён режим рассуждений. Сначала кратко объясни ход своих мыслей "
                "(2-4 предложения), затем после пустой строки дай полный ответ пользователю. "
                "Не используй специальные маркеры вроде '---' или '💭' — пиши обычным текстом. "
                "Пример структуры:\\n\\n"
                "Сначала я подумаю о... [твои рассуждения]\\n\\n"
                "[пустая строка]\\n\\n"
                "Теперь полный ответ: ... [финальный ответ]"
            )
        if search_context:
            system_parts.append(

...
class CognitiveController:
    ...
    async def _finalize_answer(self, message: str, response: str,
        ...
        if response:
            # ИСПРАВЛЕНИЕ v2: новый формат рассуждения без маркеров "---" и "💭".
            # Теперь модель пишет рассуждение в начале, затем пустая строка, затем ответ.
            # Удаляем первую часть (рассуждение) до первого двойного переноса строки,
            # если текст начинается с ключевых фраз рассуждения.
    ...
    def _build_messages(self, message: str, web_search: bool, search_context: str,
        ...
        if reasoning:
            # ИСПРАВЛЕНИЕ v3: локальные модели часто генерируют ТОЛЬКО рассуждение,
            # игнорируя инструкцию "затем дай финальный ответ". Усиливаем инструкцию,
            # требуя явного разделения между рассуждением и ответом.
            system_parts.append(
                "Включён режим рассуждений (reasoning mode). СТРОГО следуй структуре:\n"
                "1. Сначала кратко объясни ход своих мыслей (2-4 предложения) — это будет скрыто под спойлером.\n"
                "2. Затем ОБЯЗАТЕЛЬНО сделай ДВА перевода строки (пустую строку).\n"
                "3. После пустой строки дай полный финальный ответ пользователю.\n\n"
                "Пример:\n"
                "Сначала я подумаю о том, какие факторы важны для этого вопроса... [твои рассуждения]\n\n"
                "[обязательная пустая строка]\n\n"
                "Теперь мой полный ответ: ... [финальный ответ пользователю]\n\n"
                "ВАЖНО: Не используй маркеры '---', '💭', '**РАССУЖДЕНИЕ**' — только обычный текст с пустой строкой между частями."
            )
        if search_context:
            system_parts.append(

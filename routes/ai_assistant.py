...
class CognitiveController:
    ...
    def _build_messages(self, message: str, web_search: bool, search_context: str,
        ...
        if reasoning:
            # ИСПРАВЛЕНИЕ v4: Модель должна оборачивать рассуждение в теги <thought>...</thought>,
            # чтобы фронтенд мог корректно выделить этот блок и поместить его в спойлер.
            system_parts.append(
                "Включён режим рассуждений (reasoning mode). СТРОГО следуй структуре:\n"
                "1. Оберни свои размышления в теги <thought> и </thought>.\n"
                "2. После закрывающего тега </thought> сделай ДВА перевода строки (пустую строку).\n"
                "3. После пустой строки дай полный финальный ответ пользователю.\n\n"
                "Пример:\n"
                "<thought>Сначала я подумаю о том, какие факторы важны для этого вопроса... [твои рассуждения]</thought>\n\n"
                "Теперь мой полный ответ: ... [финальный ответ пользователю]\n\n"
                "ВАЖНО: Не используй маркеры '---', '💭', '**РАССУЖДЕНИЕ**' внутри текста — только теги <thought> для блока размышлений."
            )
        if search_context:
            system_parts.append(

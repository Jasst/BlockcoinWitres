...
class CognitiveController:
    ...
    def _build_messages(self, message: str, web_search: bool, search_context: str,
        ...
        if reasoning:
            # ИСПРАВЛЕНИЕ v5: Усиленная инструкция для генерации тегов <thought>.
            # Модель должна строго следовать формату: <thought>...</thought>\n\nОтвет.
            # Без тегов фронтенд не сможет выделить блок рассуждения.
            system_parts.append(
                "Включён режим рассуждений (reasoning mode). ТЫ ОБЯЗАН строго следовать этому формату:\n"
                "1. НАЧНИ ответ с открывающего тега <thought>.\n"
                "2. Напиши свои размышления внутри тега.\n"
                "3. Закрой тег </thought>.\n"
                "4. Сделай ДВА перевода строки (пустую строку).\n"
                "5. После пустой строки напиши финальный ответ пользователю.\n\n"
                "ПРИМЕР ТОГО, ЧТО ТЫ ДОЛЖЕН ВЫДАТЬ:\n"
                "<thought>Сначала я проанализирую вопрос... здесь мои мысли...</thought>\n\n"
                "Теперь мой полный ответ пользователю...\n\n"
                "КРИТИЧЕСКИ ВАЖНО: Не пиши ничего перед тегом <thought>. Не используй маркеры '---', '💭', '**РАССУЖДЕНИЕ**'. Только XML-теги."
            )
        if search_context:
            system_parts.append(
